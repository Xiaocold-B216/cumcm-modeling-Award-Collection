...
Barx1=xishu*sqrt(CircleRadial^2-(CircleRadial-1/2*BarWidth)^2);
Bary1=CircleRadial-1/2*BarWidth;
BarLength1=1/2*BoardLength-Barx1;
Angle=asin(DeskHeight/BarLength1);
BarLength=1/2*BoardLength-xishu*sqrt(CircleRadial^2-(CircleRadial-((1:BarNumber)-0.5)*BarWidth).^2);
...
for theta=linspace(0,Angle,10)
    for t= 0:pi/50:2*pi-pi/50;
        X=xishu*[CircleRadial*sin(t) CircleRadial*sin(t+pi/50) ];Y=[CircleRadial*cos(t) CircleRadial*cos(t+pi/50)];Z=[zeros(length(t)) zeros(length(t))];
        plot3(X,Y,Z)
        hold on
    end
