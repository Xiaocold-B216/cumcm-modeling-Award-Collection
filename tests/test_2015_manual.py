...
def test_05_source_snapshot_unmodified():
 s=json.loads((AN/'00_control/2015_source_hash_snapshot.json').read_text(encoding='utf-8-sig')); assert s['source_modified_count']==0 and len(s['files'])==29
...
def test_22_six_pack_metadata_matches_csv():
 for r in docs():
  m=json.loads((AN/'02_documents/document_cards/2015'/r['logical_document_id']/'metadata.json').read_text(encoding='utf-8-sig')); assert m['logical_document_id']==r['logical_document_id'] and m['role']==r['role']
def test_23_card_preferred_rep_matches_relation():
 ...
 for d,i in p.items(): assert json.loads((AN/'02_documents/document_cards/2015'/d/'metadata.json').read_text(encoding='utf-8-sig'))['preferred_representation_id']==i
...
def test_29_quality_gate_conditional_and_blocked():
 g=json.loads((AN/'08_quality/gates/2015_gate.json').read_text(encoding='utf-8-sig')); assert g['status']=='conditional_pass' and g['blocking_manual_review_items']==2
